...
public class HereRoutingClient {
  ...
  public long fetchDistanceMeters(double originLat, double originLng, double destinationLat, double destinationLng) {
    ...
    return parseRouteDistanceMeters(body);
  }

  private long parseRouteDistanceMeters(String rawJson) {
    try {
      JsonNode root = objectMapper.readTree(rawJson);
      JsonNode routes = root.path("routes");
      if (!routes.isArray() || routes.isEmpty()) {
        throw new IllegalStateException("HERE response does not contain routes");
      }
      JsonNode sections = routes.get(0).path("sections");
      if (!sections.isArray() || sections.isEmpty()) {
        throw new IllegalStateException("HERE response does not contain route sections");
      }
      long totalMeters = 0L;
      for (JsonNode section : sections) {
        JsonNode summary = section.path("summary");
        JsonNode length = summary.get("length");
        if (length == null || length.isNull()) {
          continue;
        }
        if (length.isNumber()) {
          totalMeters += Math.max(0L, length.asLong(0));
          continue;
        }
        if (length.isObject()) {
          long value = Math.max(0L, length.path("value").asLong(0));
          String unit = length.path("unit").asText("m");
          if ("km".equalsIgnoreCase(unit) || "kilometers".equalsIgnoreCase(unit)) {
            totalMeters += Math.round(value * 1000.0);
          } else {
            totalMeters += value;
          }
        }
      ...
      if (totalMeters <= 0L) {
        throw new IllegalStateException("HERE response does not contain positive route length");
      }
      return totalMeters;
    } catch (Exception ex) {
      throw new IllegalStateException("Failed to parse HERE route summary", ex);
    }
  }
