...
- Связанные документы:
  ...
  - `docs/system.md`.
- **Заменяет (для реализации):** `docs/specs/freight-cost-scenario-nbu-pricing.md` — помечен как **устаревший**, не реализуется.
...
- Расчёт пробега по странам (`POST .../country-breakdown`) — **опциональный** вход для промпта; не является предусловием запуска Gemini-расчёта (в отличие от устаревшего NBU-ТЗ).
...
- Детерминированный серверный калькулятор с формулами, курсами НБУ, справочником дорог по странам (см. устаревший `freight-cost-scenario-nbu-pricing.md`).
...
---
*Версия документа: 1.0. Актуальная спецификация расчёта фрахта; заменяет для реализации `freight-cost-scenario-nbu-pricing.md`.*
